...
if m.prplyInfo <> invalid
if invalid = m.prplyInfo["hls_url"]
hls_url = m.readUrlFromAA(m.prplyInfo, ["manifest", "manifestUrl"])
if invalid <> hls_url
m.prplyInfo["hls_url"] = hls_url
end if
end if
...
if invalid <> m.prplyInfo["hls_url"] then return m.prplyInfo
end if
return {error:"No Valid response"}
end function
impl.setStreamInfo = function(streamInfo as object) as void
if invalid = m["prplyInfo"] then m.prplyInfo = {}
for each key in ["tracking_url", "hls_url"]
...
if 0 = emtBreak.startTimeInSeconds
adBreak.renderSequence = "preroll"
else
adBreak.renderSequence = "midroll"
end if
adBreak.renderTime = emtBreak.startTimeInSeconds
...
ret = m.streamManager.compareSegment({sequence:ss.segSequence+1,
...
if appendPod then m.pendingPods.push(nab)
end for
end function
strmMgr.compareSegment = function(params as object) as object
ret = {}
