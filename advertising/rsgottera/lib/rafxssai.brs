...
function RAFX_getOTTeraHLSPlugin(params as object) as Object
    ...
    strmMgr.onMetadata = function(msg as object, useStitched as boolean, sgnode as object) as void
        ...
        if invalid <> ext_x_marker
            ...
            if m.isPodBegin(xobj.xtype)
                ...
                crrntPosWasAhead = false
                while eopByMarker <= m.crrntPosSec
                    crrntPosWasAhead = true
    ...
    strmMgr.getFillerMediaFile = function() as String
        randomNumber = Rnd(1000)
        randomNumberString = randomNumber.toStr()
        randomMediaFileID = "mf_" + randomNumberString
        return "<MediaFile id=" + chr(34) + randomMediaFileID + chr(34) + " delivery=" + chr(34) + "progressive" + chr(34) + " type=" + chr(34) + "video/mp4" + chr(34) + " bitrate=" + chr(34) + "2500" + chr(34) + " width=" + chr(34) + "1920" + chr(34) + " height=" + chr(34) + "1080" + chr(34) + " scalable=" + chr(34) + "true" + chr(34) + " maintainAspectRatio=" + chr(34) + "true" + chr(34) + "><![CDATA[https://www.ottera.tv/fake" + randomNumberString + ".mp4]]></MediaFile>"
    end function

    strmMgr.getPlaceholderMediaFile = function() as String
        return "<MediaFile delivery=" + chr(34) + chr(34) + " type=" + chr(34) + chr(34) + " width=" + chr(34) + "0" + chr(34) + " height=" + chr(34) + "0" + chr(34) + "></MediaFile>"        
    end function

    strmMgr.parseAdBeginRAF = function(xmlstr as string, xobj as object) as void
        adIface = m.sdk.getRokuAds()
        validvast = xmlstr.split("</VAST>")[0] + "</VAST>"
        if Instr(1, validvast, m.getPlaceholderMediaFile()) > 0
            validvast = validvast.replace(m.getPlaceholderMediaFile(), m.getFillerMediaFile())
        else if Instr(1, validvast, "<MediaFiles>") <= 0 and Instr(1, validvast, "</Linear>") > 0
            fillerMediaFiles = "<MediaFiles>" + m.getFillerMediaFile() + "</MediaFiles></Linear>"
            validvast = validvast.replace("</Linear>", fillerMediaFiles)
        endif
        if Instr(1, validvast, "delivery=" + chr(34) + chr(34)) > 0
            validvast = validvast.replace("delivery=" + chr(34) + chr(34), "delivery=" + chr(34) + "progressive" + chr(34))
        endif
        if Instr(1, validvast, "type=" + chr(34) + chr(34)) > 0
            validvast = validvast.replace("type=" + chr(34) + chr(34), "type=" + chr(34) + "video/mp4" + chr(34))
        endif
        ...
        if xobj.data <> invalid and type(xobj.data) = "roArray" and xobj.data.count() > 0
            xobj.data[0].xid = m.getXid(xobj)
            xobj.data[0].idid = xobj.id
        else
            print "RAFXSSAI - Invalid Parse Ad Begin RAF"
        end if
    end function
...
function RAFX_SSAI(params as object) as object
    if invalid <> params and invalid <> params["name"]
        p = {__version__:"0.0.0"}
        p = RAFX_getOTTeraHLSPlugin(params)'
        p["__version__"] = "0b.0.0.1"
